...
- [cloud-run-ai-agents/README.md](./cloud-run-ai-agents/README.md) - Complete AI Agents deployment guide
- Backend: Standard Cloud Run deployment from source
- Frontend: Standard GCS bucket deployment
...
### Writing
```
GET    /api/writing/themes             # Get themes (20)
GET    /api/writing/prompts/:age/:theme # Get personalized prompt
POST   /api/writing/submit             # Submit writing
GET    /api/writing/submissions        # Get submissions (paginated)
GET    /api/writing/submissions/:id    # Get submission details
...
### Media
```
POST   /api/media/generate-image              # Generate image with style selection (100 credits)
POST   /api/media/generate-video-from-text    # Generate video from text with style (500 credits, 8s)
GET    /api/media/:submissionId               # Get media
GET    /api/media/status/:mediaId             # Check generation status
...
### Frontend not loading or SPA routes returning 404

**Problem**: Website shows blank page, old version, or routes like `/writing` or `/submissions/{id}` return 404

**Status**: ✅ **FIXED** - GCS bucket configured to serve index.html for all 404 errors

**How it works**: GCS bucket website configuration set to:
- Main page: `index.html`
- Error page (404): `index.html`

This allows React Router to handle client-side routing for all paths.

**Solution if issues return**:
1. **Clear browser cache** - Hard refresh (Ctrl+Shift+R or Cmd+Shift+R)
2. **Verify files in GCS bucket** - Frontend files are in `fun-writing-frontend-prod`:
   ```bash
   gsutil ls gs://fun-writing-frontend-prod/
   ```
3. **Check GCS bucket website configuration** - Verify 404 error page is set:
   ```bash
   gsutil web get gs://fun-writing-frontend-prod
   # Should show: {"mainPageSuffix": "index.html", "notFoundPage": "index.html"}
   ```
4. **Re-apply website configuration if needed**:
   ```bash
   gsutil web set -m index.html -e index.html gs://fun-writing-frontend-prod
   ```
5. **Deploy frontend** - Run the deployment script:
   ```bash
   cd 03-gcs-cloud-cdn-frontend && ./deploy.sh
   ```
6. **Invalidate CDN cache** - After deployment:
   ```bash
   gcloud compute url-maps invalidate-cdn-cache fun-writing-frontend-lb --path="/*"
   ```
7. **Wait for CDN propagation** - Takes 1-2 minutes for changes to reach all edge locations

**Testing SPA routes**:
```bash
# All these should return index.html (not 404)
curl https://writing.c6web.com/writing
curl https://writing.c6web.com/submissions
curl https://writing.c6web.com/dashboard
curl https://writing.c6web.com/admin/dashboard
```

### API requests failing (CORS errors, 404)

**Problem**: Browser console shows CORS errors or API calls return 404

**Status**: ✅ **FIXED** - Backend API is operational

**Solution if issues persist**:
1. **Verify backend is running**:
   ```bash
   gcloud run services describe fun-writing-backend --region us-central1
   curl https://fun-writing-backend-yaildcgk6q-uc.a.run.app/api/writing/themes
   ```
2. **Check frontend API URL** - Should point to Cloud Run service:
   ```bash
   grep VITE_API_URL frontend/.env
   # Should be: https://fun-writing-backend-yaildcgk6q-uc.a.run.app/api
   ```
3. **Verify CORS configuration** - Should allow frontend origin:
   ```bash
   gcloud run services describe fun-writing-backend --region us-central1 \
...
4. **Test with curl**:
   ```bash
   curl -X POST https://fun-writing-backend-yaildcgk6q-uc.a.run.app/api/auth/login \
     -H "Content-Type: application/json" \
     -d '{"email":"demo@example.com","password":"demoinitialpassword"}'
   ```

### Navigation warning in console

**Warning**: "You should call navigate() in a React.useEffect()"

**Status**: ✅ **FIXED** - This is normal behavior in Landing.tsx where navigation redirects authenticated users away from the landing page using useEffect (lines 10-16 of Landing.tsx)

### Missing /vite.svg 404 error

**Status**: ✅ **RESOLVED** - File exists in GCS bucket but may be cached by browser

**Solution**: Clear browser cache and do a hard refresh

### DNS not resolving yet

**Problem**: `nslookup writing-api.c6web.com` shows NXDOMAIN

**Expected**: DNS takes 5-15 minutes to propagate after creation
- Run DNS script: `scripts/dns/update_backend_dns.sh`
- Wait for global propagation (check with `nslookup writing-api.c6web.com 8.8.8.8`)
- Verify: `curl https://writing-api.c6web.com/api/auth/login`
...
1. **Verify Cloud SQL is running**:
   ```bash
   gcloud sql instances describe fun-writing
   ```
2. **Check service account permissions**:
   ```bash
   gcloud projects get-iam-policy YOUR_GCP_PROJECT_ID \
     --flatten="bindings[].members" \
     --filter="bindings.members:fun-writing*"
   ```
3. **Verify Cloud SQL Proxy connection in deployment**

### 502 Bad Gateway error

**Problem**: Getting 502 errors from backend

**Solution**:
1. **Check backend service logs**:
   ```bash
   gcloud run services logs read fun-writing-backend --region us-central1 --limit 50
   ```
2. **Check backend health** - Restart if needed:
   ```bash
   gcloud run deploy fun-writing-backend --region us-central1 \
     --image gcr.io/YOUR_GCP_PROJECT_ID/fun-writing-backend:latest
   ```
3. **Wait for Cloud Run to start** - Can take 1-2 minutes after deployment

### AI Agents HTTP service not responding

**Problem**: Writing submissions not getting feedback or media generation fails

**Solution**:
1. **Check AI Agents service status**:
   ```bash
   gcloud run services describe fun-writing-ai-agents --region us-central1
   ```
2. **View service logs**:
   ```bash
   gcloud run services logs read fun-writing-ai-agents --region us-central1 --limit 50
   ```
3. **Test health endpoint**:
   ```bash
   SERVICE_URL=$(gcloud run services describe fun-writing-ai-agents \
...
4. **Check environment variables**:
   ...
   ```
5. **Verify API keys are set** (GOOGLE_API_KEY or GEMINI_API_KEY)
6. **Restart service if needed**:
   ```bash
   cd cloud-run-ai-agents && ./deploy.sh
   ```
